...
namespace TradingConsole.Wpf.ViewModels
{
    public partial class MainViewModel : INotifyPropertyChanged, IDisposable
    {
        ...
        public GexViewModel GexAnalysis { get; }
        ...
        private async Task LoadOptionChainOnlyInternalAsync(string apiSecurityId, string apiSegment)
        {
            try
            {
                ...
                var tempOptionChainRows = new List<OptionChainRow>();
                ...
                foreach (var strikeInfo in strikesToDisplay)
                {
                    ...
                    tempOptionChainRows.Add(new OptionChainRow
                    {
                ...
                await Application.Current.Dispatcher.InvokeAsync(() =>
                {
                    foreach (var row in tempOptionChainRows)
                    {
                        OptionChainRows.Add(row);
                    }
                    CalculateOptionChainAggregates();
                    GexAnalysis.UpdateGexData(OptionChainRows, UnderlyingPrice);
                });
        ...
        private void CalculateFocusedGexLevels(IEnumerable<OptionChainRow> rows)
        {
            decimal totalCallGex = 0;
            decimal totalPutGex = 0;
            decimal maxGexLevel = 0;
            decimal maxGexValue = 0;

            foreach (var row in rows)
            {
                var callGex = row.CallOption.Gamma * row.CallOption.OI;
                var putGex = row.PutOption.Gamma * row.PutOption.OI;

                totalCallGex += callGex;
                totalPutGex -= putGex;

                if (callGex + putGex > maxGexValue)
                {
                    maxGexValue = callGex + putGex;
                    maxGexLevel = row.StrikePrice;
                }
            }

            decimal netGex = totalCallGex + totalPutGex;
            decimal gexFlipPoint = (from row in rows
                                    orderby row.StrikePrice
                                    where (row.CallOption.Gamma * row.CallOption.OI) > (row.PutOption.Gamma * row.PutOption.OI)
                                    select row.StrikePrice).LastOrDefault();

            var niftyResult = _analysisService.GetResultForSymbol("Nifty 50");
            if (niftyResult != null)
            {
                niftyResult.MaxGexLevel = maxGexLevel;
                niftyResult.GexFlipPoint = gexFlipPoint;
                niftyResult.NetGex = netGex;
            }
        }
